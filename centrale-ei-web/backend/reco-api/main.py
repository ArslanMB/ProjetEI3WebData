...
def movie_to_vector(movie, all_genres, all_directors):
    ...
    # Genre (binaire)
    for g in all_genres:
        vec.append(1 if movie.get("genre") == g else 0)
    # Réalisateur (binaire)
...
async def get_recommendations(user_id: int) -> List[dict]:
    async with httpx.AsyncClient() as client:
        ...
        # 4. Construire la liste des genres et réalisateurs uniques
        all_genres = sorted(set(m.get("genre") for m in all_movies if m.get("genre")))
